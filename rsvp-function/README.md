...
relation-полем «Приглашение» прямо в строке гостя, поэтому функция читает коллекцию
«Гости» и фильтрует по этому id. Дополнительно читает «Приглашения» (read-only) ради
поля `Congratulation` — кастомного обращения для Hero.
```json
{ "inv": "<id приглашения>",
  "congratulation": "Дорогие Оля и Влад, ждём вас!",
  "guests": [
    { "guestId": "…", "name": "Ольга", "answered": false,
      "attending": "", "drinks": "", "drinkList": [], "comment": "" } ] }
```
`congratulation` — кастомное обращение из «Приглашения».Congratulation, либо `""`,
если поле пустое (тогда обращение фронт собирает из имён гостей: «А», «А и Б»,
«А, Б и В»). Чтение «Приглашений» — best-effort: сбой не ломает выдачу гостей,
просто вернётся `""`. `404` — приглашение с таким id не найдено (нет гостей со связью).
...
- не удаляет ничего;
- пишет только в «Гости»; «Приглашения» — лишь read-only ради `Congratulation`
  (обе коллекции захардкожены, другие документы недоступны);
